...
ctr_to_plot = [1 3];                % CRTs to plot on the graphs
...
crt_tsc_crt_bitmask = ctr_raw(:,1)==0;
ctr_raw(crt_tsc_crt_bitmask,:)=[];
...
% Cumulate PCM counters for all domain ----------------------------------------
...
% Cumulate CRT counters for all domain ----------------------------------------
disp('- Cumulate counters for all domain');
all_domain_bitmask = ctr_raw(:,3) ~= idle_domain; % bitmask: domain_id data

% TODO: use a loop on interesting CRTi here
global_ctr(1).raw = ctr_raw(all_domain_bitmask,[1 5]);
[global_ctr(1).ctr_ts, global_ctr(1).ctr_cumulated_ts] = cumulate_and_resample(global_ctr(1).raw(:,1), global_ctr(1).raw(:,2), 1, resample_delta, tests_length);

global_ctr(2).raw = ctr_raw(all_domain_bitmask,[1 6]);
[global_ctr(2).ctr_ts, global_ctr(2).ctr_cumulated_ts] = cumulate_and_resample(global_ctr(2).raw(:,1), global_ctr(2).raw(:,2), 1, resample_delta, tests_length);

global_ctr(3).raw = ctr_raw(all_domain_bitmask,[1 7]);
[global_ctr(3).ctr_ts, global_ctr(3).ctr_cumulated_ts] = cumulate_and_resample(global_ctr(3).raw(:,1), global_ctr(3).raw(:,2), 1, resample_delta, tests_length);
...
for domain_id = unique_domain_ids
    ...
    counter_domain(i).ctr(1).raw = ctr_raw(domain_bitmask,[1 5]);
    [counter_domain(i).ctr(1).ctr_ts, counter_domain(i).ctr(1).ctr_cumulated_ts] = cumulate_and_resample(counter_domain(i).ctr(1).raw(:,1), counter_domain(i).ctr(1).raw(:,2), 1, resample_delta, tests_length);
    counter_domain(i).ctr(1).ctr_percent_ts = counter_domain(i).ctr(1).ctr_ts./global_ctr(1).ctr_ts;
    
    counter_domain(i).ctr(2).raw = ctr_raw(domain_bitmask,[1 6]);
    [counter_domain(i).ctr(2).ctr_ts, counter_domain(i).ctr(2).ctr_cumulated_ts] = cumulate_and_resample(counter_domain(i).ctr(2).raw(:,1), counter_domain(i).ctr(2).raw(:,2), 1, resample_delta, tests_length);
    counter_domain(i).ctr(2).ctr_percent_ts = counter_domain(i).ctr(2).ctr_ts./global_ctr(2).ctr_ts;
    
    counter_domain(i).ctr(3).raw = ctr_raw(domain_bitmask,[1 7]);
    [counter_domain(i).ctr(3).ctr_ts, counter_domain(i).ctr(3).ctr_cumulated_ts] = cumulate_and_resample(counter_domain(i).ctr(3).raw(:,1), counter_domain(i).ctr(3).raw(:,2), 1, resample_delta, tests_length);
    counter_domain(i).ctr(3).ctr_percent_ts = counter_domain(i).ctr(3).ctr_ts./global_ctr(3).ctr_ts;
...
% Plot Package Energy and Power (RAPL), with CTRi for every domain
disp('- Plot Package Energy and Power (RAPL), with CTRi for every domain');
figure;
total_plots = 1+length(unique_domain_ids);
...
for domain_id = unique_domain_ids
    ...
    for current_ctr = ctr_to_plot
        ...
        legend_index=j;
        legendInfo{legend_index} = ['dom-' int2str(counter_domain(i).id) '-crt' int2str(current_ctr)];
        j=j+1;
    end
...
% Distribution of Package Energy consumption per domain, for every CTRi
disp('- Plot distribution of Package Energy consumption per domain, for every CTRi');
figure;
total_plots = 1+length(ctr_to_plot);
...
for current_ctr = ctr_to_plot
    ...
    contributions_matrix = [];
    for domain_id = unique_domain_ids
        contributions_matrix = [contributions_matrix, counter_domain(j).ctr(current_ctr).ctr_percent_ts.data];
        legend_index=j;
        legendInfo{legend_index} = ['dom-' int2str(counter_domain(j).id)];
        j=j+1;
    end
    ...
    contributions_matrix(sum(contributions_matrix,2)>contribution_matrix_filter,:) = [];
    area(contributions_matrix);
    title(['Reference: CTR' int2str(current_ctr)]);
    legend(legendInfo);
    xlabel('Time (s)');
    ylabel('Contribution to the total consumption (%)');
    grid on;
