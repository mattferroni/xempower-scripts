...
#ADDED LINE FOR OTHER PMCs
#echo "0x0100f004  CPU%(cpu)d  %(tsc)d (+%(reltsc)8d)  event (0x%(event)016x)  dom%(1)d, vCPU%(2)d, UOPS_RETIRED.ALL=%(3)8d, UOPS_ISSUED.ANY=%(4)8d, UOPS_ISSUED.ANY-stalls=%(5)8d, RESOURCE_STALLS.ANY=%(6)8d" > $CURRENT_FOLDER/pmc_2_trace_human.format
#echo "0x0100f004  %(tsc)d,%(cpu)d,%(1)d,%(2)d,%(3)d,%(4)d,%(5)d,%(6)d" > $CURRENT_FOLDER/pmc_2_trace_matlab.format
#echo "TSC,CPU,DOM,vCPU,UOPS_RETIRED.ALL,UOPS_ISSUED.ANY,UOPS_ISSUED.ANY-stalls,RESOURCE_STALLS.ANY" > $CURRENT_FOLDER/pmc2.csv
