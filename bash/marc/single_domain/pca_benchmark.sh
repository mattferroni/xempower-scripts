...
CPU_FOLDER=$CURRENT_FOLDER/CPU
MEM_FOLDER=$CURRENT_FOLDER/MEM
IO_FOLDER=$CURRENT_FOLDER/IO
HDD_FOLDER=$CURRENT_FOLDER/HDD
CPU_IO_FOLDER=$CURRENT_FOLDER/CPU_IO
CPU_HDD_FOLDER=$CURRENT_FOLDER/CPU_HDD
MEM_IO_FOLDER=$CURRENT_FOLDER/MEM_IO
MEM_HDD_FOLDER=$CURRENT_FOLDER/MEM_HDD
...
while [  $COUNTER -lt 10 ]; do
	TEST_FOLDER=$CPU_FOLDER/$COUNTER
	if [ ! -d $TEST_FOLDER ]; then
		mkdir -p $TEST_FOLDER
	fi

	MAPPING_FILE=$TEST_FOLDER"/domain_mapping.csv"
	WATTSUP_OUTPUT_TMP=$TEST_FOLDER"/watts-up-tmp"
	WATTSUP_OUTPUT=$TEST_FOLDER"/wattsup-watts"	
	...
	$START_XENTRACE_SCRIPT $TEST_FOLDER
	START=$(date +%s.%N)
	...
	ssh marc@10.0.0.2 'screen -d -m stress --cpu 2 --timeout 60s'
...
COUNTER=0
while [  $COUNTER -lt 10 ]; do
	TEST_FOLDER=$MEM_FOLDER/$COUNTER
	if [ ! -d $TEST_FOLDER ]; then
		mkdir -p $TEST_FOLDER
	fi
	...
	echo $(date +%s.%N) " - Sending commands to Marc domain..."
	ssh marc@10.0.0.2 'screen -d -m stress --vm 2 --timeout 60s'
	echo $(date +%s.%N) " - Command sent"
