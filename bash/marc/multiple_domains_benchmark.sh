...
MARC1_CFG=$HOME/vm/marc/marc.cfg
MARC1_NAME=$(awk '$1=="name"{print $3}' $MARC1_CFG | sed 's/.//;s/.$//')
MARC2_CFG=$HOME/vm/marc/marc2.cfg
MARC2_NAME=$(awk '$1=="name"{print $3}' $MARC2_CFG | sed 's/.//;s/.$//')
...
echo $(date +%s.%N) " - Starting benchmarks Marc1..."

echo $(date +%s.%N) " - Launching Marc1 domain..."
sudo xl create $MARC1_CFG
sudo xl vcpu-pin $MARC1_NAME 0 1
sudo xl vcpu-pin $MARC1_NAME 1 2
MAPPING_MARC=$(sudo xl list $MARC1_NAME |awk '{print $2}' | sed -n 2p)
echo "$MARC1_NAME,"$MAPPING_MARC >> $MAPPING_FILE
echo $(date +%s.%N) " - Marc1 domain started"
...
echo $(date +%s.%N) " - Starting benchmarks Marc2..."

echo $(date +%s.%N) " - Launching Marc2 domain..."
sudo xl create $MARC2_CFG
sudo xl vcpu-pin $MARC2_NAME 0 1
MAPPING_MARC=$(sudo xl list $MARC1_NAME |awk '{print $2}' | sed -n 2p)
echo "$MARC2_NAME,"$MAPPING_MARC >> $MAPPING_FILE
echo $(date +%s.%N) " - Marc2 domain started"

sleep 60

echo $(date +%s.%N) " - Sending commands to Marc domain..."
ssh marc@10.0.0.3 'screen -d -m /home/marc/stress_iterative_benchmark_v2'
echo $(date +%s.%N) " - Command sent"
