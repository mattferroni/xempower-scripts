...
MARC_1_CFG=$HOME/vm/marc/marc.cfg
MARC_2_CFG=$HOME/vm/marc/marc_2.cfg
...
CURRENT_FOLDER=$TESTS_FOLDER/multi-domains-redis-ffmpeg-$NOW
...
sudo xl vcpu-pin marc 0 3
MAPPING_MARC=$(sudo xl list marc-2 |awk '{print $2}' | sed -n 2p)
echo "marc-2,"$MAPPING_MARC >> $MAPPING_FILE
echo $(date +%s.%N) " - Marc domain started"
...
ssh marc@10.0.0.5 'screen -d -m /home/marc/redis_benchmark_multidomain.sh'
echo $(date +%s.%N) " - Command sent"

sleep 500s

echo $(date +%s.%N) " - Sending commands to Marc domain..."
ssh marc@10.0.0.2 'screen -d -m /home/marc/ffmpeg_benchmark_multidomain.sh'
echo $(date +%s.%N) " - Command sent"
